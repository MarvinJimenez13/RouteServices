...
import com.udemy.routeservices.Presenters.driver.MapDriverPresenterImpl;
...
import com.udemy.routeservices.Views.MainActivity;
...
public class MapDriverActivity extends AppCompatActivity implements OnMapReadyCallback, MapDriverView {
    ...
    private LocationListener locationListenerGPS = location -> {
        ...
        mEndLatLng = currentLatLng;

        if(mStartLatLng != null)
            CarMoveAnim.carAnim(marker, mStartLatLng, mEndLatLng);
            ...
            for (Location location : locationResult.getLocations()) {
                if (getApplicationContext() != null) {
                    if (!mIsStartLocation) {//SIGNIFICA QUE YA RECONOCIO LA UBICACION POR PRIMERA VEZ
                        ...
                        //mueve la camara al centro del icono, con ANIMATE CAMARA EN LIGAR DE MOVE CAMARA
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        //VALIDAR SI SE DEJO CONECTADO O NO
        if(SharedPreferencesUber.getInstance(MapDriverActivity.this).getConnectionStatusDriver())
            startLocation();
        else
            disconnect();
    ...
    public void onClick(){
        if(isConnect){
            disconnect();
            SharedPreferencesUber.getInstance(MapDriverActivity.this).saveConnectionDriver(false);
        }else{
            SharedPreferencesUber.getInstance(MapDriverActivity.this).saveConnectionDriver(true);
            startLocation();
        }
    }
